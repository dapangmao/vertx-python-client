...
class MockServer:
    ...
    def run(self):
        coro = asyncio.start_server(self.handler, '127.0.0.1', 8765, loop=self.loop)
...
def test_client_functionality():
    logging.debug("Will set up")
    server = MockServer()
    server.run()

    client = EventBus(host="127.0.0.1", port=8765)
